...
class WaxDrop_UI_Init():
    def ui_setup(self):
        ...
        mode_container = win_tools.add(ui.UI_Container()) # TODO: make this rounded

        mode_frame = mode_container.add(ui.UI_Frame('Wax Drop Mode'))
        wax_mode = mode_frame.add(ui.UI_Options(mode_getter, mode_setter, separation=0))
        wax_mode.add_option('Sketch', value='sketch wait')
        wax_mode.add_option('Paint', value='paint wait')

        finish_container = win_tools.add(ui.UI_Container())
        finish_frame = finish_container.add(ui.UI_Frame('Finish Tools'))
        finish_frame.add(ui.UI_Button('Fuse and Continue', self.fuse_and_continue, align=0))
        finish_frame.add(ui.UI_Button('Commit', self.done, align=0))
        finish_frame.add(ui.UI_Button('Cancel', lambda:self.done(cancel=True), align=0))

        #####################################
        ### Collapsible Help and Options   ##
        #####################################
        info = self.wm.create_window('Wax Dropper Help', 
                                     {'pos':9, 
                                      'movable':True, 
        ...
        action = opts.add(ui.UI_Options(get_action, set_action, label="Action: ", vertical=True))
        action.add_option("add")
        action.add_option("subtract")
        action.add_option("none")

        surface = opts.add(ui.UI_Options(get_surface_target, set_surface_target, label="Surface: ", vertical=True))
